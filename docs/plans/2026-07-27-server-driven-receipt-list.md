...
struct ServerDateFormatterTests {
    ...
    ///
    /// Deriving the expected value from `ServerDateFormatter` itself would make
    /// this `f(x) == f(x)` — true against any implementation, including a broken
    /// one. Ground truth has to come from somewhere else.
    @Test
